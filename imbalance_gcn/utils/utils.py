...
def bcubed(ref_labels, sys_labels, cm=None):
    ...
    # cal pre mostly
    cm_max_idx=np.argmax(cm,axis=1)
    cm_sum=cm.sum(axis=0)
    pre=[]
    for i in range(cm.shape[0]):
        pre.append(cm[i][cm_max_idx[i]]/cm_sum[cm_max_idx[i]])
    #np.save('pre_.npy',pre)
    cm_norm = cm / cm.sum()
    precision = np.sum(cm_norm * (cm / cm.sum(axis=0)))
    #np.save('pre.npy', cm_norm * (cm / cm.sum(axis=0)))
    #np.save('cm.npy', cm)
    recall = np.sum(cm_norm * (cm / np.expand_dims(cm.sum(axis=1), 1)))
    #np.save('rec.npy', cm_norm * (cm / np.expand_dims(cm.sum(axis=1), 1)))
