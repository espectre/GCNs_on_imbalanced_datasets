...
def connected_components_constraint_adaptive(nodes, max_sz, score_dict=None, th=None):
    '''
    only use edges whose scores are above `th`
    if a component is larger than `max_sz`, all the nodes in this component are added into `remain` and returned for next iteration.
    '''
...
def graph_propagation_thresh(edges, scores, labels):
    gather_edges = edges
    gather_scores =scores
    accs = dict()
    for i in range(1, 10):
        i = i*0.1
        print('Now is thresh: ', i)
        cnt = 0
        for idx in range(len(gather_scores)):
            score = gather_scores[idx]
            label1 = labels[gather_edges[idx][0]]
            label2 = labels[gather_edges[idx][1]]
            if (label1 == label2 and score >= i) or (label1 != label2 and score < i):
                cnt += 1
        print(cnt)
        acc = round(cnt / gather_scores.shape[0], 4)
        accs[acc] = i
    max_thresh = accs[max(list(accs.keys()))]

    G = nx.Graph()
    G.add_nodes_from(list(range(len(labels))))
    for idx in range(len(gather_scores)):
        score = gather_scores[idx]
        if score >= max_thresh:
            G.add_edge(gather_edges[idx][0], gather_edges[idx][1])

    G_clusters = list(nx.connected_components(G))
    ...
    for i in range(len(labels)):
        final_pred.append(G.nodes[i]['pred'])
