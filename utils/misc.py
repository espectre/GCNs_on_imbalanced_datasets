...
def read_label_npy(fn_npy, start_pos=0, verbose=True):
    lb2idxs = {}
    idx2lb = {}
...
def clusters_pickle2txts(fn):
    data = load_pkl(fn)
    n_clusters = len(data)
    # folder_name = os.path.splitext(os.path.basename(fn))[0]
    folder_name = os.path.splitext(fn)[0]
    if not os.path.isdir(folder_name):
        os.mkdir(folder_name)
    for i in range(n_clusters):
        save2txt(folder_name,i,data[i])


def save2txt(folder,index,cluster):
    file_name='cluster'+str(index)+'.txt'
    file_path=os.path.join(folder,file_name)
    print('save cluster %d to '.format(index), file_path)
    with open(file_path, 'w') as of:
        for face_id in cluster:
            of.write(str(face_id) + '\n')
...
def dump2npy(ofn, data, force=False):
    if os.path.exists(ofn+'.npy') and not force:
        return
    np.save(ofn, data)
...
def dump_data(ofn, data, force=False, verbose=False):
    ...
    if ofn.endswith('.json'):
        ...
        print("Dump data to a npy file: ", ofn+'.npy')
        dump2npy(ofn, data, force=force)
